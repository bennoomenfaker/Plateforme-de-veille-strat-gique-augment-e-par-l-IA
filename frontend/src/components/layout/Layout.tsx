...
const navItems = [
  ...
  { path: '/graph', label: 'Graph', icon: 'M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4m0 5c0 2.21-3.582 4-8 4s-8-1.79-8-4' },
        ...
        <div className="px-3 py-4 border-t" style={{ borderColor: '#1e2535' }}>
          <Link to="/profile"
            ...
            <div className="w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0 overflow-hidden"
              style={{ background: 'linear-gradient(135deg,#3b82f6,#6366f1)' }}>
              {user?.photo_url
                ? <img src={user.photo_url.startsWith('http') ? user.photo_url : `/api${user.photo_url}`} alt="" className="w-full h-full object-cover" />

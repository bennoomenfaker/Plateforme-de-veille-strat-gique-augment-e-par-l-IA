...
export class ObjectivesService {
  ...
  async updateObjective(objectiveId: string, userId: string, data: any) {
    ...
    const updated = await this.prisma.projectObjective.update({
      ...
      data: { 
        ...
        priority: data.priority !== undefined ? data.priority : objective.priority 
      },
    });

    await this.logActivity(userId, 'UPDATE_OBJECTIVE', 'objective', objectiveId);
    return updated;
  }
  ...
  async deleteObjective(objectiveId: string, userId: string) {
    ...
    await this.logActivity(userId, 'DELETE_OBJECTIVE', 'objective', objectiveId);

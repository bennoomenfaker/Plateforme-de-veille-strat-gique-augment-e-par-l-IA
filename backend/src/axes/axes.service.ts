...
export class AxesService {
  ...
  private async checkObjectiveAccess(objectiveId: string, userId: string) {
    ...
    if (project.organisation?.members.some(m => m.user_id === userId && m.statut === 'ACTIF')) return objective;
  ...
  async createAxis(objectiveId: string, userId: string, data: any) {
    ...
    const count = await this.prisma.projectAxis.count({ where: { objective_id: objectiveId } });
    if (count >= 5) throw new BadRequestException('Maximum 5 axes par objectif');
